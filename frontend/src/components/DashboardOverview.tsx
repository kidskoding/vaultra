...
function NoScoreCard({ connected }: { connected: boolean }) {
  return (
    <div className="bg-[#292524] rounded-2xl border border-[#44403c] p-6 hover:border-[#da7756]/30 transition-colors animate-fade-in">
      <p className="text-sm font-medium text-[#a8a29e] mb-3">Funding Readiness Score</p>
      <p className="text-sm text-[#78716c]">
        {connected
          ? "We're analyzing your QuickBooks data. Your funding readiness score will appear here soon."
          : 'Connect QuickBooks to calculate your funding readiness score.'}
      </p>
    </div>
  );
}
...
export default function DashboardOverview() {
  ...
  const connected = data.quickbooks?.connected ?? false;
  ...
  return (
    <div className="space-y-8 mt-6">
      ...
      {loading ? (
        <ScoreSkeleton />
      ) : connected && data.readiness ? (
        <ScoreCard data={data.readiness} />
      ) : (
        <NoScoreCard connected={Boolean(data.quickbooks?.connected)} />
      )}
      ...
      {loading ? (
        <StatsSkeleton />
      ) : connected && data.metrics ? (
        <StatsGrid metrics={data.metrics} />
      ) : (
        <div className="bg-[#292524] rounded-2xl border border-[#44403c] p-6 text-sm text-[#a8a29e] animate-fade-in">
          Connect QuickBooks to see your live revenue, chargeback, and transaction metrics.
        </div>
      )}
